...
<table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-top:10px;">
	...
	<c:forEach var="product" items="${list}">
		...
		<tr class="ct_list_pop">
			...
			<td></td>
			<c:if test="${ fn:trim(product.proTranCode)=='0' || fn:trim(product.proTranCode)=='3' }">
				<td align="left"><a href="/getProduct.do?prodNo=${ product.prodNo }&menu=${ requestScope.menu }">${ product.prodName}</a></td>
			</c:if>
			<c:if test="${ fn:trim(product.proTranCode)!='0' && fn:trim(product.proTranCode)!='3' }">
